...
public class TransactionalRegion extends HRegion {
  ...
  /**
   * Open HRegion. Calls initialize and sets sequenceid to both regular WAL and
   * trx WAL.
   * 
   * @param reporter
  ...
  protected void doReconstructionLog(final Path oldCoreLogFile,
    ...
    Path trxPath = new Path(oldCoreLogFile.getParent(),
        THLog.HREGION_OLD_THLOGFILE_NAME);
  ...
  public Result get(final long transactionId, final Get get) throws IOException {
    ...
    if (localGet != null) {
      LOG.trace("Transactional get of something we've written in the same transaction "
          + transactionId);
   ...
   */
  public InternalScanner getScanner(final long transactionId, final Scan scan)
      throws IOException {
    checkClosing();
    ...
    if (LOG.isDebugEnabled()) {
      ...
      if (numRemoved > 0) {
        ...
        if (!commitedTransactionsBySequenceNumber.isEmpty()) {
          debugMessage.append(" Still have [")
  ...
  public void setTransactionLog(THLog trxHLog) {
    this.transactionLog = trxHLog;
  }

  public void setTransactionalLeases(Leases transactionalLeases) {
